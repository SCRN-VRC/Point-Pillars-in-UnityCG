Shader "PointPillars/TestCompact"
{
    Properties
    {
        ...
        Pass
        {
            ...
            int2 ActiveTexelIndexToUV(float index)
            {
                ...
                while (uv.z >= 1)
                {
                    ...
                    // if (index == 0.0 && uv.z == 8)
                    //     buffer[0] = float4(count00, count01, count10, activeTexelSumInPreviousLods);
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                int2 uv = ActiveTexelIndexToUV(UVToIndex(i.uv.xy * _ActiveTexelMap_TexelSize.zw));   
                if (uv.x == -1)
                {
                    return 0;
                }
                return _WeightsTex[uv];
            }
            ENDCG
        }
    }
